...
class TalkinBot:
    ...
    def _music_download(self,query):
        ...
        outdir=BASE_DIR/"generated_music"; outdir.mkdir(parents=True,exist_ok=True)
        opts={"quiet":True,"no_warnings":True,"noplaylist":True,"format":"bestaudio/best","outtmpl":str(outdir/"%(id)s_%(epoch)s.%(ext)s"),"socket_timeout":35,"retries":4,"fragment_retries":4,"cachedir":False,"overwrites":True,"postprocessors":[{"key":"FFmpegExtractAudio","preferredcodec":"mp3","preferredquality":"192"}],"http_headers":{"User-Agent":"Mozilla/5.0 Chrome/131 Safari/537.36"}}
        if YOUTUBE_COOKIE_FILE: opts["cookiefile"]=YOUTUBE_COOKIE_FILE
        with yt_dlp.YoutubeDL(opts) as ydl:
            info=ydl.extract_info(query if re.match(r"^https?://",query,re.I) else "ytsearch1:"+query,download=True)
            if info and info.get("entries"): info=info["entries"][0]
            duration=int(info.get("duration") or 0)
            if duration>MUSIC_MAX_SECONDS: raise RuntimeError(f"الأغنية أطول من {MUSIC_MAX_SECONDS} ثانية")
            mp3s=sorted(outdir.glob("*.mp3"),key=lambda x:x.stat().st_mtime,reverse=True)
            if not mp3s: raise RuntimeError("تم العثور على الأغنية لكن لم يتم إنشاء ملف MP3")
            return info,mp3s[0]
